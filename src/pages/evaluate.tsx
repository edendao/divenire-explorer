...
// sections
import { GeneratorListItem, EvaluateNavConfig } from '../sections/evaluate';
...
export default function ClaimsIndex() {
  ...
  useEffect(() => {
    if (!type) {
      push(PATH_EVALUATE.list('generators'));
    }
  }, [type]); // eslint-disable-line

  return (
    <AuthGuard>
      <Page title="Evaluate: Dashboard">
        <Container maxWidth={false}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={4}>
            <Card sx={{ width: 240 }}>
              <NavSectionVertical pb={3} navConfig={EvaluateNavConfig} />
            </Card>
            <Box flex="1">
